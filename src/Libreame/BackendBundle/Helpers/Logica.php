...
class Logica {
    ...
    public function ejecutaAccion($solicitud, EntityManager $em) {
        try {
            ...
            switch ($tmpSolicitud) {
                ...
                case GamesController::txAccElimiOfe: {//Dato:18 : Cancela / Elimina oferta
                        //echo "<script>alert('Antes de entrar a Cancelar trato-".$solicitud->getEmail()."')</script>";
                        //$objGestEjemplares = $this->get('gest_ejemplares_service');
                        $respuesta = GestionEjemplares::cancelarTrato($solicitud, $em);
                        break;
                    }
    ...
    public static function generaRespuesta(Respuesta $respuesta, Solicitud $pSolicitud, $parreglo, $em) {
        ...
        try {
            ...
            //echo "ingresa a recuperar respuesta : accion : ".$pSolicitud->getAccion()."\n"; 
            ...
            switch ($pSolicitud->getAccion()) {
                ...
                case GamesController::txAccRecOpera:  //Dato: 5
                    //echo "Respuesta de recuperar operación \n"; 
                ...
                //accion de Cancelar / eliminar oferta
                case GamesController::txAccElimiOfe:  //Dato: 18
                    $JSONResp = Logica::respuestaCancelarOferta($respuesta, $pSolicitud, $parreglo);
                    break;
    ...
    public static function respuestaDatosUsuario(Respuesta $respuesta, Solicitud $pSolicitud, $parreglo, EntityManager $em) {
        ...
        try {
            ...
            $nomgenero = ManejoDataRepository::getNomGenero($usuario->getinusugenero());
            ...
            return array('idsesion' => array('idaccion' => $pSolicitud->getAccion(),
                ...
                'idrespuesta' => array('respuesta' => $respuesta->getRespuesta(),
                    'usuario' => array('idusuario' => $usuario->getIdusuario(),
                        ...
                        'usugenero' => array('idgenero' => $usuario->getinusugenero(), 'nomgenero' => $nomgenero),
                        ...
                        'usufecnac' => $fecha,
                        'usulugar' => array('inlugar' => $lugar->getinlugar(), 'nomlugar' => $lugar->gettxlugnombre()),
 ...
 */
    ...
    /*
     * respuestaCancelarOferta: 
     * Funcion que genera el JSON de respuesta para la accion de cancelarOferta:: GamesController::txAccElimOfer

     */

    public function respuestaCancelarOferta(Respuesta $respuesta, Solicitud $pSolicitud, $parreglo) {
        try {
            return array('idsesion' => array('idaccion' => $pSolicitud->getAccion(),
                    'idtrx' => '', 'ipaddr' => $pSolicitud->getIPaddr()),
                'idrespuesta' => (array('respuesta' => $respuesta->getRespuesta())));
    ...
    public function respuestaVerDetalleTrato(Respuesta $respuesta, Solicitud $pSolicitud, $parreglo, $em) {
        try {
            ...
            //echo "Entra a genera la respuesta VerDetalleTrato \n";
            foreach ($parreglo as $detalle) {
                ...
                $arrTmp[] = array("id" => $detalle->getidactividadusuario(),
                    "usrescribe" => array("idusuario" => $usrescribe->getidusuario(), 
                        "nomusuario" => utf8_encode($usrlee->gettxnomusuario()),
                        "usrlee" => array("idusuario" => $usrlee->getidusuario(), "nomusuario" => utf8_encode($usrescribe->gettxnomusuario()),
                        "fecha" => $detalle->getactusuariofecha(),
                        "mensaje" => utf8_encode($detalle->getactusuariomensaje()),
            ...
            $arrEjemplar[] = array("idejemplar" => $ejemplar->getidejemplar(),
                ...
                "nomvideojuego" => utf8_encode($vj->gettxnomvideojuego()));
